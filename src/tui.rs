...
impl App {
    ...
    pub fn get_value(&self, key: &str) -> String {
        match key {
            ...
            "candidate_range" => self.config.candidate_range.to_string(),
            "melody_force" => self.config.melody_force.to_string(),
            "use_joint_scoring" => self.config.use_joint_scoring.to_string(),
    ...
    pub fn update_value(&mut self) {
        if let Some(i) = self.state.selected() {
            ...
            match key {
                ...
                "candidate_range" => if let Ok(v) = self.input_buffer.parse() { self.config.candidate_range = v; },
                "melody_force" => if let Ok(v) = self.input_buffer.parse() { self.config.melody_force = v; },
                "use_joint_scoring" => if let Ok(v) = self.input_buffer.parse::<i32>() { self.config.use_joint_scoring = v == 1; },
